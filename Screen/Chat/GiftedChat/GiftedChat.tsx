...
const GiftedChat: React.FC<GiftedChatProps> = props => {
  const {
    messages = [],
    initialText = '',
    isTyping,
  ...
  // 2025-01-01 : 여기서 에러 발생한 경우 babel.config.js에 하기의 것을 추가해서 해결을 했다.
  ...
  const handleTextInputFocusWhenKeyboardShow = useCallback(() => {
    console.log('handleTextInputFocusWhenKeyboardShow...');
    if (
...
export const GiftedChatAppend = <TMessage extends IMessage>(
...
export const GiftedChatPrepend = <TMessage extends IMessage>(
  currentMessages: TMessage[] = [],
  messages: TMessage[],
  inverted = true,
) => {
